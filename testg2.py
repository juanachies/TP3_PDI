            ...
            for i in range(1, num_labels):
                ...
                if 300 < area < 3000 and 0.6 < aspect_ratio < 1.2:
                    ...
                    if circles is not None:
                        for c in circles[0]:
                            cx, cy, r = c.astype(int)

                            y1 = max(0, cy - r)
                            y2 = min(roi_th.shape[0], cy + r)
                            x1 = max(0, cx - r)
                            x2 = min(roi_th.shape[1], cx + r)

                            region = roi_th[y1:y2, x1:x2]

                            if np.mean(region) > 120:
                                num_puntos += 1

                    if num_puntos < 1 or num_puntos > 6:
                        num_puntos = 0
...
for i in range(1, 5):
    detectar_dados(f'tirada_{i}.mp4')