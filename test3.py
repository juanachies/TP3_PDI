...
# ---------------------------
# Variables para detectar reposo
# ---------------------------
centroides_previos = []
frames_quietos = 0
FRAMES_REPOSO = 8          # cantidad de frames consecutivos quietos
UMBRAL_MOVIMIENTO = 2.5    # píxeles
ya_reportado = False      # para no imprimir mil veces
...
while True:
    ...
    if contours:
        ...
        # ---------------------------
        # Mostrar SOLO si están quietos
        # ---------------------------
        if en_reposo:
            if not ya_reportado:
                print("Dados detenidos:")
                for i, d in enumerate(dados_detectados):
                    print(f"{d[4]}")
                ya_reportado = True
